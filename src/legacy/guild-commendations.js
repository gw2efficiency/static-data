export default [
  {
    name: 'Guild Commendation Trader',
    items: [
      // Sentinel's Insignia & Inscription
      {id: 41556, quantity: 1, gold: 8000, commendations: 10},
      {id: 41560, quantity: 1, gold: 8000, commendations: 10},
      // WvW Stuff
      {id: 39717, quantity: 1, gold: 1200, commendations: 3},
      {id: 42004, quantity: 2, gold: 1200, commendations: 3},
      {id: 42003, quantity: 3, gold: 1200, commendations: 3},
      {id: 39715, quantity: 4, gold: 1200, commendations: 3}
    ]
  },
  {
    name: 'Guild Trader',
    description: "Requires a guild hall with unlocked 'Guild Trader'",
    items: [
      {id: 74763, quantity: 1, commendations: 1},
      {id: 72845, quantity: 1, commendations: 1},
      {id: 71428, quantity: 1, commendations: 1}
    ]
  }
]
